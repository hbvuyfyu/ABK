...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _loadStats() async {
    try {
      final resp = await ApiService.get('/admin/dashboard');
      if (resp['success'] == true) setState(() => _stats = resp['data']);
    } catch (_) {}
  ...
  Widget _buildStatsGrid() {
    final stats = [
      ...
      {'label': 'إجمالي الأرباح', 'value': '\$${(_stats?['totalRevenue'] ?? 0).toStringAsFixed(2)}', 'icon': Icons.attach_money_outlined, 'color': AppTheme.accent},
    ];
